...
## Разработка полигона (Раздел находится в разработке) {id="developer-chapter-prepare-poly"}

Первым этапом разработки полигона является расположение объектов окружения и разметки зон в TrikStudio.
...
[документации](https://help.trikset.com/studio/2d-model/restrictions) к TrikStudio.
Ограничения добавляются в специальный блок 
`constraints` в экспортированном файле.

```xml
<?xml version='1.0' encoding='utf-8'?>
<root version="20190819">
    <!-- Здесь находится описание зон и объектов -->
    <constraints>
...
На случай если условие для ограничения состоит из нескольких компонентов есть специальный блок `constraints`.
...
<event id="on_success">
	<trigger>
        <message text="Получено баллов: %\text1%" >
            <replace var="text1">
...
> В случае отсутствия такого вывода все решения будут оценены в 0 баллов со статусом "Неуспешно".
...
Также робот не должен покидать границы другой определенной зоны и остановиться в последней зоне. 
...
этих параметров остается на усмотрение читателя.

Для начала необходимо разместить зоны в TrikStudio и сохранить модель мира.
    ...
    <constraints>
        ...
        <event id="zone1_checker" settedUpInitially="true" dropsOnFire="true">
            ...
            <trigger>
                ...
                <!-- Взводим событие проверяющее посещение 2 зоны -->
        ...
        <event id="zone2_checker" settedUpInitially="false" dropsOnFire="true">
            ...
            <trigger>
                ...
                <!-- Взводим событие проверяющее посещение 3 зоны -->
        ...
        <event id="zone3_checker" settedUpInitially="false" dropsOnFire="true">
            <conditions glue="and">
                <inside objectId="robot1" regionId="zone3"/>
                <!-- Проверяем что робот выключил моторы -->
                <equals>
                    <objectState object="robot1.M3.power"/>
                    <int value="0"/>
                </equals>
            ...
            <trigger>
                ...
                <!-- Вызываем success поскольку выполнение задачи завершено -->
                <success/>
            </trigger>
        </event>
        ...
        <event id="work_zone_checker" settedUpInitially="true" dropsOnFire="true">
            ...
            <trigger>
                <!-- Вызываем success поскольку выполнение задачи завершено, так как робот покинул рабочую зону -->
                <success/>
            </trigger>
        </event>
